...
export function SummaryAnalytics({ results }: any) {
    ...
    const fetchStatistics = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `http://localhost:5000/api/statistics?${params.toString()}`,
        );
        if (response.ok) {
  ...
  const handleDownloadReport = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:5000/api/report?${params.toString()}`,
      );
      if (!response.ok) {
  ...
  const barData = stats?.fail_reasons
    ? Object.entries(stats.fail_reasons).map(([reason, count]) => ({
        name: reason,
        count,
      }))
    : [];
  ...
  return (
    <div className="space-y-6 max-w-6xl">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Popover>
            ...
            <PopoverContent
              className="w-auto p-0 bg-card border border-border"
              align="start"
            >
              <Calendar
                mode="single"
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <StatCard
          label="Pass"
          value={loading ? "..." : passCount}
          color="bg-emerald-600"
        />
        <StatCard
          label="Fail"
          value={loading ? "..." : failCount}
          color="bg-red-600"
        />
        <StatCard
          label="Pass Rate"
