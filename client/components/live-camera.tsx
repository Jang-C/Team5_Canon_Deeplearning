...
export function LiveCamera({ setIsProcessing, setResults, onDownload }: any) {
  ...
  const startCameraStream = useCallback(async () => {
    try {
      ...
      setError(
        "카메라 접근 불가: 권한을 확인하거나 다른 앱에서 사용 중이 아닌지 확인하세요.",
      );
      stopCameraStream();
      setIsStreamReady(false);
    }
      ...
      async (blob) => {
        ...
        try {
          ...
          const response = await fetch(
            "http://localhost:5000/api/analyze-frame",
            {
              method: "POST",
              body: formData,
            },
          ...
          const analyzedImageUrl =
            result.analyzed_image_base64 || result.details?.annotated_image;
          ...
          setResults((prev: any[]) => [
            {
              ...
              imageUrl: analyzedImageUrl
                ? `data:image/jpeg;base64,${analyzedImageUrl}`
                : null,
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden shadow-xl">
        <div className="aspect-video bg-gradient-to-br from-muted to-card flex items-center justify-center relative overflow-hidden">
          <video
            ...
            style={{
              filter: `brightness(${100 + brightness * 2}%) contrast(${exposure})`,
            }}
          />
      ...
      <div className="bg-card border border-border rounded-lg p-6 space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium text-muted-foreground mb-1">
            명도:{" "}
            <span className="text-foreground font-mono">{brightness}</span>
          </label>
          <input
            type="range"
            min={-BRIGHTNESS_MAX}
