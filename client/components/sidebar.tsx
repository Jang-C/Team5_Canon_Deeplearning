...
function NavItem({
  ...
  return (
    ...
    >
      {icon}
      <span
        className={`text-sm font-medium whitespace-nowrap transition-opacity duration-200 ${isCollapsed ? "opacity-0 w-0" : "opacity-100 w-auto"}`}
      >
        {label}
      </span>
    </button>
...
export function Sidebar({ isCollapsed, setIsCollapsed }: SidebarProps) {
  ...
  return (
    ...
    >
      {/* 2. 토글 버튼 영역 추가 */}
      <div
        className={`flex justify-${isCollapsed ? "center" : "end"} mb-4 ${!isCollapsed ? "pr-2" : ""}`}
      >
        <button
      ...
      <div className="mb-8 overflow-hidden">
        <div className="flex items-center gap-3 mb-2 justify-start">
          <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-blue-500 to-cyan-400 flex items-center justify-center flex-shrink-0"></div>
          {!isCollapsed && (
